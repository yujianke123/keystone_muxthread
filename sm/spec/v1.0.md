...
| `SBI_SM_SLOTTEE_DEBUG` | 2008 |Query or clear SlotTEE debug state from the host|
...
| `SBI_SM_INIT_REENTRY_TEMPLATE` | 3008 |Initialize a SlotTEE slot re-entry template from the current runtime context|
...
The host can use this SlotTEE debug-only interface to query or clear the
slot re-entry template state. It is intended for tests and diagnostics, not for
normal scheduling policy.

- Arguments:
  - `eid` -- the enclave ID.
  - `req` -- a `slottee_debug_req_t` with `SLOTTEE_DEBUG_VERSION` and an op.
  - `resp` -- a `slottee_debug_resp_t` populated by the security monitor.
- Supported ops:
  - `SLOTTEE_DEBUG_OP_REENTRY_STATUS` -- query template ready state, epoch,
    active thread count, and busy slot count.
  - `SLOTTEE_DEBUG_OP_REENTRY_CLEAR` -- clear the template and then return the
    same state fields.
- Error Code (`a0`): `SBI_ERR_SM_ENCLAVE_SUCCESS` (=0) if successful,
  otherwise an error code.
- Return Value (`a1`): current `reentry_ready` value.
...
##### Init Re-entry Template (FID #3008)

```cpp
struct sbiret sbi_sm_init_reentry_template(void)
```

An enclave runtime calls this after runtime initialization and before SlotTEE
logical-thread user entry. The security monitor captures the current S-mode
runtime CSR state as the stable template used by later slot re-entry.
