...
#define SLOTTEE_FIRST_BUSY_REVOKE_SPINS (1UL << 40)
#define SLOTTEE_FIRST_BUSY_REVOKE_CAP_SLOTS 3
...
busy_until_timer_interrupt(void)
{
  ...
  for (i = 0; i < SLOTTEE_FIRST_BUSY_REVOKE_SPINS; i++)
    asm volatile("" ::: "memory");
}

static void
first_busy_revoke_worker(void* opaque)
{
  (void)opaque;
...
eapp_entry()
{
  ...
  while (1) {
    for (slot_id = 1; slot_id <= SLOTTEE_FIRST_BUSY_REVOKE_CAP_SLOTS; slot_id++) {
      if (slottee_lt_spawn(slot_id, first_busy_revoke_worker, (void*)slot_id) !=
